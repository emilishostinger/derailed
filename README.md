...
| 🪄 **DNS records written for you** | Connect Cloudflare once and "point your domain at this server" stops being homework: pick the domain from a dropdown and the A record, www CNAME and wildcard write themselves, DNS-only so certificates keep arriving. The biggest onboarding cliff, removed. |
...
| 📈 **Visitor figures, no tracker** | Counted by the proxy that already serves every request. No script in your pages, nothing leaves the machine, nothing to consent to. And the pages people looked for and did not find, grouped and counted: "312 people tried /blog/rss this month" is a broken link with an address, not a mood. |
...
| 📋 **What will change** | Flip one switch on a project and edits to variables, settings and domains collect into "3 changes waiting, apply together?" with a diff a person can read, values never included. One honest screen between editing and production, and every variable save, reviewed or not, lands in a history of which keys moved and when. |
| 🗝️ **SSH keys, and the toggle that matters** | See every key that can open the machine, add one by paste, and turn off password login as one honest switch: the single highest-value hardening act on a VPS. It refuses to lock the door while no key could still get you in, proves the config parses before reloading, and lands first in the include order so cloud-init cannot quietly outvote it. |
| 🕵️ **Is anything leaking or known-broken?** | A daily scan with plain verdicts: live-key-shaped strings in your repos and uploads, the password nobody changed, a secret that is both a variable and a string in the files, and (with Trivy installed) known holes in the images behind your apps, wired straight to the update button. A live password reached this repo once and was caught by luck; this builds the luck in. |
| 🩺 **Health checks that speak the app's language** | One dropdown says what healthy means: it answers on its port, its answer contains a text, its port accepts a connection, a command inside it succeeds, or it just keeps running. The same answer powers deploys, uptime and alerts, and the contains check catches an app serving its error page with a straight face and a 200. |
...
| 🕳️ **A tunnel to your database from your laptop** | `derailed tunnel blog-db` and TablePlus connects to 127.0.0.1. No port opened to the internet: the server dials the database over the network it already shares with it, every byte on a connection the token and audit log have seen. The opposite of the expose button. |
...
| 🤖 **Runs from your editor** | An MCP server, so Claude Code, Cursor or Codex can deploy, read logs and add domains in the same conversation where you write the code. Your role, your audit log, the same confirmations as a person. |
...
| 🎩 **WordPress superpowers** | The most-run app on earth, given house treatment: sign in to wp-admin with one press and no password, plugin and theme updates behind the same backup-first promise as everything else, and a staging copy with its own database and files whose push-to-live backs the whole project up before it writes a byte. |
| 🖼️✂️ **Pictures the right size** | Ask for /_img/photo.jpg?w=800 and it comes back that wide, re-encoded, WebP for browsers that take it. One switch, one libvips sidecar with no published ports, year-long caching in the visitor's browser, and nobody has to learn what WebP is. |
| ✏️ **Edit a file, and it's live** | Dragged-in sites get a real editor: every uploaded file, syntax highlighting for whatever language it is, and a save that publishes through the ordinary deploy pipeline. First use: one button adds a 404 or 500 page, wired up automatically just by existing at the site's root. |
